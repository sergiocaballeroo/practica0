...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	// 🔥 TU NUEVA TECLA (animación perro)
	if (key == GLFW_KEY_M && action == GLFW_PRESS)
	{
		AnimDog = !AnimDog;
	}

	// (opcional) deja esto también corregido
...
void Animation() {
	...
	if (AnimDog)
	{
		...
		// Piernas (movimiento alternado tipo caminar)
		FLegs = sin(time * 5.0f) * 30.0f;
		RLegs = -sin(time * 5.0f) * 30.0f;

		// Cola (rápido)
		tail = sin(time * 10.0f) * 25.0f;

		// Cabeza (más suave)
		head = sin(time * 2.0f) * 10.0f;
	}
}
